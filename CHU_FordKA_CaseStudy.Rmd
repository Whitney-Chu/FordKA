...
#Analyze how preferences relate to "choosers" vs "non-choosers"
```{r}
df %>%
...
```
#Analyze Preferences against Clusters
```{r}
...
#Analyze how cluster 1 relates to demographics (Non-Choosers)
```{r}
mode.pref1 <- 0
ones = df[df$preference == 1 & df$Cluster ==1,0]
...
#Analyze how cluster 2 relates to demographics (Choosers)
```{r}
mode.pref2 <- 0
...
# Hierarchical Clustering for demographic data
```{r}
...
#Itâs also possible to draw the dendrogram with a border around the 2 clusters. 
...
#Preview Pyschographics dataset
```{r}
...
#Determining Optimal Clusters using the Silhouette method
```{r}
fviz_nbclust(df2, kmeans, method = "silhouette")
```

#R squared vs Silhouette
```{r}
...
# Hierarchical Clustering for psychographic data
```{r}
